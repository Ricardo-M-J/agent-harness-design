...
**重要：必须使用 bash 工具读取图片文件**，因为 read 工具无法将图片传给多模态模型。
...
2. 使用 bash 工具读取并分析图片：
   ...
   # 将图片转为 base64 并分析
   for img in outputs/{项目名}/*.png; do
     echo "=== $img ==="
     base64 -w 0 "$img" | head -c 500
     echo ""
   done
   ```
...
4. 输出评审报告到 outputs/{项目名}/review_round1.md
```
...
4. 输出评审报告到 outputs/{项目名}/review_round2.md
```
